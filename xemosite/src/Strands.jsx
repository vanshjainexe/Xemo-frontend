...
void main() {
  ...
  float screenX = (gl_FragCoord.x - 0.5 * uResolution.x) / (0.5 * uResolution.x);
  float env = pow(max(cos(screenX * 3.14159265 * 0.5), 0.0), uTaper);
  ...
  for (int i = 0; i < ${MAX_STRANDS}; i++) {
    ...
    float y = w * amp + (fi - float(uStrandCount - 1) * 0.5) * 0.045 * uSpread * env;

    float d = abs(uv.y - y);
    float thick = (0.001 + 0.05 * e) * (0.05 + 1.5 * env) * uThickness;
